...
export default function EditAppointmentPage({ params: paramsPromise }) {
  ...
  // Fetch configs, booking details
  useEffect(() => {
    const loadAllDetails = async () => {
      if (!bookingId || !admin?._id) return;
      setIsLoading(true);
      try {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const res = await updateAdminBookingRecord(bookingId, {
  ...
  return (
    <>
      <PageMeta title="Edit Appointment - Booking Admin" description="Edit user appointment details" />
      <PageBreadcrumb
        items={[
          { label: "Home", to: "/" },
