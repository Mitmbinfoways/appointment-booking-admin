...
export default function AdminsPage() {
  ...
  useEffect(() => {
    ...
    const actionP =
      searchParams?.get("action") || searchParams?.get("create") || "";
  ...
  const handleToggleCredentials = async (adminId) => {
    try {
      ...
      if (res.status === 200 && res.data?.statusCode === 200) {
        Toast({
          message: res.data.message || "Credentials visibility updated",
          type: "success",
        });
        fetchAdmins();
      } else {
        Toast({
          message:
            res.data?.message || "Failed to toggle credentials visibility",
          type: "error",
        });
      }
    } catch (err) {
      ...
      Toast({
        message: "Failed to toggle credentials visibility.",
        type: "error",
      });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.status === 201 && res.data?.statusCode === 201) {
        ...
        Toast({
          message: res.data?.message || "Failed to create Admin",
          type: "error",
        });
      }
    } catch (err) {
  ...
  const handleEditSubmit = async (e) => {
    ...
    if (editFormData.phoneNumber && editFormData.phoneNumber.trim() !== "") {
      if (
        editFormData.phoneNumber.length !== 10 ||
        !/^\d{10}$/.test(editFormData.phoneNumber)
      ) {
        errors.phoneNumber = "Phone number must be 10 digits";
    ...
    try {
      ...
      if (res.status === 200 && res.data?.statusCode === 200) {
        ...
        Toast({
          message: res.data?.message || "Failed to update Admin",
          type: "error",
        });
      }
    } catch (err) {
      ...
      const errMsg =
        err?.response?.data?.message || "Error updating Admin account.";
      Toast({ message: errMsg, type: "error" });
    }
  };
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      const errMsg =
        err?.response?.data?.message || "Error deleting Admin account.";
      Toast({ message: errMsg, type: "error" });
    }
  };
  ...
  return (
    <>
      <PageMeta
        title="Admins - SuperAdmin Control Panel"
        description="Manage sub-admin profiles"
      />
      <PageBreadcrumb
        items={[
          { label: "Home", to: "/" },
        ...
        <div className="flex flex-col gap-4 p-4 border-b border-gray-200 sm:flex-row sm:items-center sm:justify-between sm:p-6">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Create, review, edit, and toggle active status of sub-admin
              accounts.
            </p>
          </div>
                    ...
                    <TD>
                      <button
                        ...
                        className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 ${
                          admin.showApiCredentials
                            ? "bg-blue-600"
                            : "bg-gray-200"
                        }`}
                        title={
                          admin.showApiCredentials
                            ? "API Credentials Visible on Admin Profile (Click to Hide)"
                            : "API Credentials Hidden from Admin Profile (Click to Show)"
                        }
                      >
                        <span
      ...
      <CustomModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <div className="w-full">
          <div className="border-b border-gray-200 pb-4 mb-4">
            ...
            <p className="text-sm text-gray-500">
              Add a new admin account to deploy their booking slots.
            </p>
          </div>
      ...
      >
        <div className="w-full">
          <div className="border-b border-gray-200 pb-4 mb-4">
            ...
            <p className="text-sm text-gray-500">
              Update admin credentials and business details.
            </p>
          </div>
