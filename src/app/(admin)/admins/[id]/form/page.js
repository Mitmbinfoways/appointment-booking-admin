...
export default function AdminFormConfigPage({ params: paramsPromise }) {
  ...
  const fetchFormConfig = async () => {
    ...
    try {
      ...
      if (res.status === 200 && res.data?.statusCode === 200) {
        ...
        const sorted = (res.data.data.fields || []).sort(
          (a, b) => a.order - b.order,
        );
  ...
  const handleSubmit = async (e) => {
    ...
    for (const f of formFields) {
      ...
      if (
        ...
        Toast({
          message: `Please add at least one option for field "${f.label}".`,
          type: "error",
        });
        return;
      }
    ...
    setIsSaving(true);
    try {
      const res = await updateAdminFormConfigSuper(adminId, {
        fields: formFields,
      });
      if (res.status === 200 && res.data?.statusCode === 200) {
        Toast({
      ...
      console.error(err);
      Toast({
        message: "Connection error saving configuration.",
        type: "error",
      });
    } finally {
      setIsSaving(false);
  ...
  return (
    <>
      <PageMeta
        title="Configure Dynamic Form"
        description="SuperAdmin Admin Form Config Panel"
      />
      <PageBreadcrumb
        items={[
          { label: "Home", to: "/" },
        ...
        <div className="flex items-center justify-between border-b border-gray-200 pb-4 mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Configure dynamic inputs by adding fields and dragging them to
              reorder.
            </p>
          </div>
          <Button
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          {isLoading ? (
            ...
            <div className="text-center py-12 text-sm text-gray-400 border-2 border-dashed border-gray-200 rounded-lg">
              No custom form fields created. Click "Add Field" to construct a
              dynamic form schema.
            </div>
          ) : (
