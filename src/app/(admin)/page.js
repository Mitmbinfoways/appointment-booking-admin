...
import { getDashboardStats, getBookings, getAdminsList } from "@/config/AxiosConfig";
...
const StatCard = ({ title, value, icon: Icon, color, onClick }) => {
  ...
  return (
    <div
      onClick={onClick}
      className={`rounded-lg border-2 p-5 transition-all duration-300 ease-in-out hover:shadow-md hover:-translate-y-0.5 ${currentTheme.cardBg
    ...
    >
      <div className="flex items-start justify-between">
        ...
        <div className={`p-3.5 rounded-lg flex items-center justify-center shrink-0 ${currentTheme.iconBg}`}>
...
export default function Dashboard() {
      ...
      if (isSuperAdmin) {
        ...
        try {
          ...
          if (resAdmins.status === 200 && resAdmins.data?.statusCode === 200) {
            ...
            const inactiveCount = list.filter((a) => a.isActive === false).length;
            setSuperAdminStats((prev) => ({
              ...prev,
        ...
        // Fetch Regular Admin Recent Bookings
        try {
          const resBookings = await getBookings();
          if (resBookings.status === 200 && resBookings.data?.statusCode === 200) {
            const allBookings = resBookings.data.data?.bookings || [];
            setRecentBookings(allBookings.slice(0, 6));

            const counts = { confirmed: 0, pending: 0, cancelled: 0, total: allBookings.length };
  ...
  return (
    <>
      ...
      />
      <PageBreadcrumb items={[{ label: "Home", to: "/" }, { label: "Dashboard", to: "/" }]} />
      ...
      <div className="pb-16 space-y-6">
            ...
            <div className="flex items-center gap-2">
              {isSuperAdmin ? (
                <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-purple-100 text-purple-700 border border-purple-200 flex items-center gap-1">
                  <Crown className="w-3.5 h-3.5 text-amber-500" /> SuperAdmin Panel
                </span>
              ) : (
              ...
              <span className="text-xs text-gray-500">Overview of your business activities</span>
            </div>
            <h1 className="text-2xl font-extrabold text-gray-900 mt-1">
              {isSuperAdmin ? "SuperAdmin Dashboard" : `Welcome back, ${admin?.name || "Admin"} 👋`}
            </h1>
          </div>
        ...
        {isSuperAdmin ? (
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-gray-900 group-hover:text-white">Create New Admin</h4>
                      <p className="text-xs text-gray-500 group-hover:text-blue-100">Add tenant account</p>
                    </div>
                  </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-gray-900 group-hover:text-white">Manage Admins</h4>
                      <p className="text-xs text-gray-500 group-hover:text-purple-100">Toggle & Edit status</p>
                    </div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-gray-400 group-hover:text-white" />
                </button>
            ...
            <div className="bg-white rounded-lg border border-gray-200/80 shadow-xs overflow-hidden">
              <div className="p-5 border-b border-gray-200/80 flex items-center justify-between">
                <div>
                  <h3 className="text-base font-bold text-gray-900">Registered Admins Directory</h3>
                  <p className="text-xs text-gray-500 mt-0.5">Overview of active admin accounts</p>
                </div>
                <Button
                  variant="primary"
                  onClick={() => router.push("/admins")}
                  ...
                  <TBody>
                    {isLoading ? (
                      ...
                      adminsList.map((adm, idx) => (
                        <TR key={adm._id || idx}>
                          <TD className="text-xs text-gray-500 font-medium">{idx + 1}</TD>
                          <TD className="text-xs font-bold text-gray-900">
                            {adm.name || adm.userName || "Admin Account"}
                          </TD>
                          <TD className="text-xs text-gray-600">{adm.email || "-"}</TD>
                          <TD className="text-xs text-gray-600">{adm.mobileNumber || adm.phone || "-"}</TD>
                          <TD>
                            <span
                              className={`inline-flex px-2.5 py-0.5 text-[11px] font-semibold border rounded-full ${adm.isActive !== false
          ...
          /* ------------------------------------------------------------- */
          /* REGULAR ADMIN DASHBOARD VIEW (Shown ONLY when logged in as Admin) */
          /* ------------------------------------------------------------- */
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
              ...
              <StatCard
                ...
                value={formatNumber(stats.pendingBookings || statusCounts.pending || 0)}
                icon={AlertCircle}
                color="orange"
            ...
            <div className="bg-white rounded-lg border border-gray-200/80 p-6 shadow-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <button
                  onClick={() => router.push("/appointments-list/create-appointment")}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-gray-900 group-hover:text-white">Create Booking</h4>
                      <p className="text-xs text-gray-500 group-hover:text-indigo-100">Schedule new appointment</p>
                    </div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-gray-400 group-hover:text-white" />
                </button>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-gray-900 group-hover:text-white">All Appointments</h4>
                      <p className="text-xs text-gray-500 group-hover:text-blue-100">View & edit list</p>
                    </div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-gray-400 group-hover:text-white" />
                </button>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-gray-900 group-hover:text-white">Holidays Calendar</h4>
                      <p className="text-xs text-gray-500 group-hover:text-purple-100">Set breaks & off days</p>
                    </div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-gray-400 group-hover:text-white" />
                </button>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-gray-900 group-hover:text-white">Slot Settings</h4>
                      <p className="text-xs text-gray-500 group-hover:text-slate-200">Configure time slots</p>
                    </div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-gray-400 group-hover:text-white" />
                </button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 bg-white rounded-lg border border-gray-200/80 shadow-xs overflow-hidden flex flex-col">
                <div className="p-5 border-b border-gray-200/80 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">Latest bookings received</p>
                  </div>
                  <Button
                    variant="outline"
                    size="sm"
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-lg border border-gray-200/80 p-6 shadow-xs">
                  <h3 className="text-base font-bold text-gray-900 mb-1">Status Breakdown</h3>
                  <p className="text-xs text-gray-500 mb-4">Appointments status overview</p>
                      ...
                      <div className="flex justify-between text-xs font-semibold mb-1.5">
                        ...
                        <span className="text-gray-900 font-bold">
                          ...
                          <span className="text-gray-400 font-normal">
                            ({statusCounts.total > 0 ? Math.round((statusCounts.confirmed / statusCounts.total) * 100) : 0}%)
                          </span>
                        </span>
                      </div>
                    ...
                    <div>
                      <div className="flex justify-between text-xs font-semibold mb-1.5">
                        ...
                        <span className="text-gray-900 font-bold">
                          ...
                          <span className="text-gray-400 font-normal">
                            ({statusCounts.total > 0 ? Math.round((statusCounts.pending / statusCounts.total) * 100) : 0}%)
                          </span>
                        </span>
                      </div>
                      <div className="w-full bg-gray-100 h-2.5 rounded-full overflow-hidden">
        ...
        {selectedBooking && (
          <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
            <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 flex items-center justify-between">
              <div>
                <span className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">Booking ID</span>
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="p-3 rounded-lg bg-gray-50 border border-gray-200">
                <span className="block text-[10px] text-gray-400 uppercase font-semibold">Appointment Date</span>
                <span className="font-bold text-gray-900">{formatDateDDMMYYYY(selectedBooking.slotDate)}</span>
              </div>
              <div className="p-3 rounded-lg bg-gray-50 border border-gray-200">
                <span className="block text-[10px] text-gray-400 uppercase font-semibold">Slot Time</span>
                <span className="font-bold text-gray-900">{selectedBooking.slotStartTime} - {selectedBooking.slotEndTime}</span>
              </div>
            </div>

            {selectedBooking.dynamicResponses && (
              <div className="space-y-2 pt-2">
                <h4 className="text-xs font-bold text-gray-700 uppercase tracking-wider">Submitted Form Details</h4>
                <div className="bg-gray-50 rounded-lg p-3.5 border border-gray-200 space-y-2">
                  {Object.entries(selectedBooking.dynamicResponses).map(([key, val]) => (
                    <div key={key} className="flex flex-col sm:flex-row sm:justify-between sm:items-center text-xs border-b border-gray-200/60 pb-1.5 last:border-b-0 last:pb-0">
            ...
            <div className="flex justify-end pt-3 border-t border-gray-200">
              <Button variant="secondary" size="md" onClick={() => setIsViewModalOpen(false)}>
                Close
              </Button>
            </div>
          </div>
